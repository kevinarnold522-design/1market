...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!allowedTypes.includes(content_type)) return Response.json({ error: 'Unsupported media type' }, { status: 400, headers: corsHeaders });

    const bytes = Uint8Array.from(atob(base64_data), c => c.charCodeAt(0));
    if (bytes.byteLength > 25 * 1024 * 1024) return Response.json({ error: 'File must be 25MB or smaller' }, { status: 400, headers: corsHeaders });
    ...
    if (!bucket || !accessKeyId || !secretAccessKey) return Response.json({ error: 'Cloudflare R2 is not configured' }, { status: 500, headers: corsHeaders });
    ...
    return Response.json({ success: true, file_url, object_key: key, bucket, content_type, size: bytes.byteLength }, { headers: corsHeaders });
  } catch (error) {
