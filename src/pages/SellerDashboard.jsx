...
import { Plus, Pencil, Trash2, X, Save, ArrowLeft, Package, MapPin, Store, Search, ExternalLink, ChevronDown, User, LogOut, BarChart2, Upload, Heart } from 'lucide-react';
...
function Field({ label, value, onChange, type = 'text', placeholder = '', required = false }) {
  return (
    <div>
      <label className="block font-body text-xs font-semibold text-white/50 mb-1">{label}{required && <span className="text-red-400 ml-0.5">*</span>}</label>
      {type === 'textarea' ? (
        <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
          className="w-full border border-white/10 rounded-xl px-3 py-2 font-body text-sm resize-none h-24 focus:outline-none focus:border-[#00D4FF] bg-white/5 text-white placeholder-white/25" />
      ) : (
...
function AIPriceSearch({ title, brand, onClose }) {
  ...
  const search = async () => {
    ...
    try {
      const res = await base44.integrations.Core.InvokeLLM({
        prompt: `Search the web and find current selling prices for: "${title}"${brand ? ` by ${brand}` : ''}. Look across Lazada Philippines, Shopee Philippines, Amazon, eBay. Return structured price comparison data.`,
        add_context_from_internet: true,
        response_json_schema: {
          type: 'object',
          properties: {
            product_name: { type: 'string' },
            platforms: { type: 'array', items: { type: 'object', properties: { platform: { type: 'string' }, price_php: { type: 'string' }, url: { type: 'string' }, condition: { type: 'string' }, notes: { type: 'string' } } } },
      ...
      setResults({ error: 'Could not fetch price data.' });
    }
    setLoading(false);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-[#0A192F]/85 backdrop-blur-sm" onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.93, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.93 }}
        onClick={e => e.stopPropagation()} className="w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl"
        ...
        <div className="p-5">
          <p className="font-body text-xs text-white/50 mb-4">Searching: <strong className="text-white">{title}</strong></p>
          {loading && (
          ...
          {results && !loading && (
            <div className="space-y-3">
              {results.error ? <p className="text-red-400 font-body text-sm">{results.error}</p> : (
                <>
                    ...
                    {(results.platforms || []).map((p, i) => (
                      <div key={i} className="flex items-center justify-between p-2.5 rounded-xl bg-white/5 border border-white/8">
                        ...
                        <div className="text-right">
                          ...
                          {p.url && p.url !== 'N/A' && (
                            <a href={p.url} target="_blank" rel="noopener noreferrer" className="font-body text-[9px] text-[#2563EB] flex items-center gap-0.5 justify-end mt-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>
                          )}
                        </div>
                      </div>
                    ))}
...
function SellerSetupModal({ user, onSave, onClose }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    await base44.auth.updateMe({ is_seller: true, seller_location: location, seller_area: area, member_type: 'seller', account_type: 'business_owner' });
    setSaving(false);
    onSave();
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
        ...
        <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-2"><Store className="w-4 h-4 text-[#00D4FF]" /><h3 className="font-heading font-bold text-white">Set Up Your Store</h3></div>
...
function ImageUploader({ images, onAdd, onRemove }) {
  ...
  return (
    <div>
      <label className="block font-body text-xs font-semibold text-white/50 mb-2">Product Photos</label>
      <div className="flex flex-wrap gap-2 mb-2">
        {images.map((url, i) => (
          <div key={i} className="relative group">
            <img src={url} alt="" className="w-16 h-16 rounded-xl object-cover border border-white/10" />
            <button onClick={() => onRemove(i)} className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-red-500 text-white text-[10px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">✕</button>
          </div>
        ))}
        <label className={`w-16 h-16 rounded-xl border-2 border-dashed border-white/20 flex flex-col items-center justify-center cursor-pointer hover:border-[#00D4FF]/50 transition-colors ${uploading ? 'opacity-50' : ''}`}>
          {uploading ? <div className="w-5 h-5 border-2 border-[#00D4FF]/30 border-t-[#00D4FF] rounded-full animate-spin" /> : <><Upload className="w-4 h-4 text-white/30" /><span className="font-body text-[9px] text-white/30 mt-1">Upload</span></>}
          <input type="file" accept="image/*" className="hidden" onChange={handleFile} disabled={uploading} />
        </label>
      </div>
      <p className="font-body text-[9px] text-white/25">Upload product photos. First photo will be the main image.</p>
    </div>
  );
}
...
export default function SellerDashboard() {
  ...
  const openAdd = () => { setForm({ ...EMPTY, seller_name: user?.full_name || '', area: user?.seller_area || '', location: user?.seller_location || 'Manila' }); setEditing(null); setShowForm(true); };
  const openEdit = (item) => { setForm({ ...EMPTY, ...item, price: String(item.price || ''), year: String(item.year || '') }); setEditing(item); setShowForm(true); };
  ...
  const save = async () => {
    ...
    const uploadedImages = form.extra_images || [];
    // If image_url is empty but we have extra_images, use first as main
    const mainImage = form.image_url || uploadedImages[0] || '';
    const extras = mainImage && uploadedImages[0] === mainImage ? uploadedImages.slice(1) : uploadedImages;
    ...
    const data = {
      ...
      image_url: mainImage,
      extra_images: extras,
      seller_name: user?.full_name || form.seller_name,
    };
    ...
    return (
      <div className="min-h-screen bg-[#070F1A] flex items-center justify-center">
        ...
        <div className="relative z-10 text-center p-6">
          ...
          <button onClick={() => base44.auth.redirectToLogin(window.location.href)} className="px-6 py-3 bg-[#00D4FF] text-[#0A192F] rounded-xl font-body font-bold">Sign In / Sign Up</button>
        </div>
      </div>
    );
  }
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 backdrop-blur-xl" style={{ background: 'rgba(7,15,26,0.92)' }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-lg bg-[#00D4FF] flex items-center justify-center"><span className="text-[#0A192F] font-bold text-[10px]">1</span></div>
          ...
          <div className="flex items-center gap-2">
            {user?.seller_location ? (
              ...
              <button onClick={() => setShowSetup(true)} className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 font-body text-[10px] font-semibold hover:bg-amber-500/20 transition-colors">
                <MapPin className="w-3 h-3" /> Set Location
              </button>
            )}
            <div className="relative">
              <button onClick={() => setProfileOpen(p => !p)} className="flex items-center gap-2 px-2.5 py-1.5 rounded-xl bg-white/8 border border-white/10 hover:border-[#00D4FF]/40 transition-all">
              ...
              <AnimatePresence>
                {profileOpen && (
                  <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.95 }}
                    ...
                    <div className="p-2">
                      <Link to="/profile" onClick={() => setProfileOpen(false)} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/10 text-white/60 hover:text-white font-body text-xs transition-colors">
                        <User className="w-3.5 h-3.5 text-[#00D4FF]" /> My Profile
                      </Link>
                      <button onClick={() => { setProfileOpen(false); setShowSetup(true); }} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/10 text-white/60 hover:text-white font-body text-xs transition-colors">
                        <MapPin className="w-3.5 h-3.5 text-amber-400" /> Store Location
                      </button>
                      <button onClick={() => base44.auth.logout('/')} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-red-500/10 text-red-400 font-body text-xs transition-colors">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 flex gap-1 pb-2">
          {[{ key: 'listings', label: 'My Listings', icon: Package }, { key: 'favourites', label: 'Favourites', icon: Heart }, { key: 'analytics', label: 'Analytics', icon: BarChart2 }].map(({ key, label, icon: Icon }) => (
          ...
          <button onClick={openAdd} className="ml-auto flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#00D4FF] hover:bg-white text-[#0A192F] font-body text-xs font-bold transition-colors">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 pt-28 pb-12">
        ...
        {user && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 flex-wrap mt-0.5">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#00D4FF]/15 text-[#00D4FF] border border-[#00D4FF]/20">
                    🏪 Business Owner
                  </span>
            ...
            {!user.seller_location && (
              <button onClick={() => setShowSetup(true)} className="flex items-center gap-2 px-4 py-2 bg-amber-500/10 border border-amber-500/30 text-amber-400 rounded-xl font-body text-xs font-semibold hover:bg-amber-500/20 transition-colors">
                <MapPin className="w-3.5 h-3.5" /> Set Store Location
              </button>
            )}
          </motion.div>
        )}
        ...
        {activeNav === 'analytics' && (
          <div className="rounded-2xl p-8 text-center" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <p className="font-body text-sm text-white/40">View listing views, clicks, and contact rates.</p>
          </div>
        )}
        ...
        {activeNav === 'favourites' && (
          <div>
            <h3 className="font-heading font-bold text-white text-lg mb-4">Saved / Favourites</h3>
            {favourites.length === 0 ? (
              <div className="rounded-2xl p-8 text-center" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                <Heart className="w-10 h-10 text-white/20 mx-auto mb-3" />
                <p className="font-body text-sm text-white/40">No saved items yet. Browse listings and tap ❤️ to save.</p>
                <Link to="/buysell" className="inline-block mt-4 px-5 py-2.5 bg-[#2563EB] text-white rounded-xl font-body font-bold text-sm hover:bg-[#00D4FF] hover:text-[#0A192F] transition-colors">Browse Listings →</Link>
              </div>
            ) : (
              <div className="grid sm:grid-cols-2 gap-3">
                {favourites.map(fav => (
                  <div key={fav.id} className="rounded-xl border border-white/8 p-3 flex items-center gap-3" style={{ background: 'rgba(255,255,255,0.03)' }}>
                    {fav.image_url && <img src={fav.image_url} alt={fav.title} className="w-12 h-12 rounded-xl object-cover flex-shrink-0 border border-white/10" />}
                    <div className="flex-1 min-w-0">
                      <p className="font-heading font-bold text-sm text-white truncate">{fav.title}</p>
                      <p className="font-body text-xs text-[#00D4FF]">{fav.price_label}</p>
                      <p className="font-body text-[10px] text-white/30">{fav.area} · {fav.category}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <>
            <AnimatePresence>
              {showForm && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  className="mb-6 rounded-2xl border border-white/10 p-5 space-y-4" style={{ background: 'rgba(13,31,60,0.9)' }}>
                  ...
                  {/* Image uploader */}
                  <ImageUploader
                    images={form.extra_images || (form.image_url ? [form.image_url] : [])}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Field label="Title" value={form.title} onChange={v => setF('title', v)} placeholder="e.g. Samsung Galaxy S24" required />

                    <div>
                      <label className="block font-body text-xs font-semibold text-white/50 mb-1">Category <span className="text-red-400">*</span></label>
                    ...
                    <div>
                      <label className="block font-body text-xs font-semibold text-white/50 mb-1">Subcategory</label>
                    ...
                    <Field label="Area / District" value={form.area} onChange={v => setF('area', v)} placeholder="e.g. Bacoor, BGC" />
                    ...
                    <Field label="Model / Size" value={form.size || form.model} onChange={v => { setF('size', v); setF('model', v); }} placeholder="e.g. 256GB / US 10" />
                  </div>

                  {/* Electronics — mandatory warranty & specs */}
                  {isElectronics && (
                    <div className="rounded-xl p-4 space-y-3" style={{ background: 'rgba(0,212,255,0.05)', border: '1px solid rgba(0,212,255,0.2)' }}>
                      <p className="font-body text-xs text-[#00D4FF] font-semibold">📱 Electronics — Warranty & Specs Required</p>
                      <Field label="Warranty Details" value={form.warranty} onChange={v => setF('warranty', v)} placeholder="e.g. 1 Year Samsung PH Warranty" required />
                      <Field label="Full Specifications" value={form.specs} onChange={v => setF('specs', v)} type="textarea" placeholder="e.g. 6.7&quot; AMOLED, Snapdragon 8 Gen 3, 12GB RAM, 256GB Storage..." required />
                    </div>
                  )}
                  ...
                  <div className="flex gap-3 flex-wrap">
                    <button onClick={save} className="flex items-center gap-2 px-5 py-2.5 bg-[#00D4FF] hover:bg-white text-[#0A192F] rounded-xl font-body font-semibold text-sm transition-colors">
                      <Save className="w-4 h-4" /> {editing ? 'Update Listing' : 'Publish Listing'}
                    </button>
                    {form.title && (
                      <button onClick={() => setShowPriceSearch(true)} className="flex items-center gap-2 px-4 py-2.5 bg-white/5 border border-[#00D4FF]/30 text-[#00D4FF] rounded-xl font-body text-sm font-semibold hover:bg-[#00D4FF]/10 transition-colors">
                        <Search className="w-4 h-4" /> AI Price Check
                      </button>
                    )}
                    <button onClick={() => { setShowForm(false); setEditing(null); }} className="px-5 py-2.5 border border-white/10 text-white/50 rounded-xl font-body text-sm hover:bg-white/5 transition-colors">
                      Cancel
                    </button>
                  </div>
                </motion.div>
            ...
            {loading ? (
              ...
              <div className="text-center py-24 rounded-2xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <p className="font-body text-sm text-white/30 mb-5">Add your first product to appear on the marketplace.</p>
              ...
              <div className="space-y-2">
                {listings.map(item => (
                  <motion.div key={item.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                    className="rounded-2xl border border-white/8 p-4 flex items-center gap-4 flex-wrap" style={{ background: 'rgba(255,255,255,0.03)' }}>
                    {item.image_url ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {item.subcategory && <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-white/5 text-white/40">{item.subcategory}</span>}
                      ...
                      {item.type === 'electronics' && item.warranty && <p className="font-body text-[10px] text-[#00D4FF]/70 mt-0.5">🛡️ {item.warranty}</p>}
                    </div>
