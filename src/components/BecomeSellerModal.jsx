...
// Step 0 — What type of seller are you?
const SELLER_TYPES = [
  {
    value: 'products',
    label: 'Products & Goods',
    desc: 'I sell physical items — new or used',
    icon: 'buysell',
    color: '#8b5cf6',
  },
  {
    ...
    desc: 'I offer a skill, trade, or professional service',
    icon: 'services',
    color: '#3b82f6',
  },
  {
    value: 'jobs',
    label: 'Job Postings',
    desc: 'I post job openings or hiring opportunities',
    icon: 'jobs',
    color: '#f59e0b',
  },
  {
    value: 'rent',
    label: 'Rooms & Spaces',
    desc: 'I rent out rooms, apartments, or commercial spaces',
    icon: 'rent',
    color: '#10b981',
  },
  {
    ...
    desc: 'I list hotels, tours, vehicle rentals, or flights',
    icon: 'travel',
    color: '#0ea5e9',
  },
  {
    ...
    desc: 'I sell home-cooked meals, baked goods, or drinks',
    icon: 'food',
    color: '#f97316',
  },
];

// Step 1 — Category picker based on seller type
const CATS_BY_TYPE = {
  products: [
    ...
    { value: 'clothing', label: 'Clothing & Fashion', icon: 'clothing', subcats: ["Men's Tops", "Women's Tops", 'Bottoms', 'Outerwear', 'Activewear', 'Kids Clothing'] },
  ...
  rent: [
    { value: 'rent_lease', label: 'Rooms & Apartments', icon: 'rent_lease', subcats: ['Room for Rent', 'Apartment / Condo', 'Bedspace / Dorm', 'House for Rent'] },
...
export default function BecomeSellerModal({ user, onClose, onSuccess }) {
  const [step, setStep] = useState(0);   // 0=seller type, 1=pick categories, 2=confirm
          ...
          <div className="flex items-center justify-between px-5 pt-5 pb-3 flex-shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="font-body text-[9px] text-white/35">Step {step + 1} of 3</p>
              </div>
            </div>
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 0 && (
                <motion.div key="s0" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-5 pb-6">
                  ...
                  <p className="font-body text-sm text-white/45 mb-5">Choose the option that best describes you. This helps us show you the right listing categories.</p>

                  <div className="space-y-2">
                    {SELLER_TYPES.map(st => (
                      <motion.button key={st.value} whileTap={{ scale: 0.98 }}
                        onClick={() => {
                          setSellerType(st.value);
                          setSelected([]);
                          setError('');
              ...
              {/* STEP 1 — Pick specific categories */}
              {step === 1 && (
                <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-5 pb-6">
                  ...
                  <p className="font-body text-sm text-white/45 mb-1">
                    Pick <span className="text-emerald-400 font-bold">1–{MAX_PICKS}</span> that match what you list.
                  </p>
                    ...
                    {availableCats.map(cat => {
                      ...
                      return (
                        <motion.button key={cat.value} whileTap={{ scale: 0.98 }}
                          ...
                          className={`w-full flex items-start gap-3 p-3 rounded-2xl border-2 text-left transition-all ${isSelected ? 'border-emerald-400/60 bg-emerald-500/10' : 'border-white/10 hover:border-white/20 bg-white/4'}`}>
                          <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 transition-all ${isSelected ? 'bg-emerald-500/25' : 'bg-white/8'}`}>
                            <CategoryIcon name={cat.icon} size={18} color={isSelected ? '#34d399' : 'rgba(255,255,255,0.5)'} />
                          </div>
                          <div className="flex-1 min-w-0">
                  ...
                  <div className="sticky bottom-0 pt-2 pb-1" style={{ background: 'linear-gradient(to top,#0D1F3C,transparent)' }}>
                    <button onClick={() => {
                      if (selected.length < MIN_PICKS) { setError('Please pick at least 1 category.'); return; }
                      setError(''); setStep(2);
                    }}
                      ...
                      style={selected.length >= MIN_PICKS
                        ? { background: 'linear-gradient(135deg,#10b981,#00D4FF)', boxShadow: '0 0 20px rgba(16,185,129,0.3)' }
              ...
              {step === 2 && (
                <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-5 pb-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedCats.map(c => (
                        <span key={c.value} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-500/15 border border-emerald-400/30 text-emerald-300 font-body text-xs font-semibold">
                          <CategoryIcon name={c.icon} size={12} color="#34d399" />
                          {c.label}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="rounded-2xl p-4 mb-5 space-y-2" style={{ background: 'rgba(0,212,255,0.05)', border: '1px solid rgba(0,212,255,0.15)' }}>
                    ...
                    {[
                      'Post listings in all selected categories immediately',
                      'Your own public seller profile page',
                      'Receive and track orders from buyers',
                      'Apply for Verified Partner badge any time',
                    ].map((perk, i) => (
