...
function NodeCircle({ icon, label, sub, glowColor, delay }) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center gap-2 cursor-default z-10"
    >
      <motion.div
        ...
        style={{
          ...
          boxShadow: hovered
            ? `0 0 28px 8px ${glowColor}55, 0 0 60px 16px ${glowColor}22`
            : `0 0 14px 2px ${glowColor}33`,
        }}
      >
      ...
      <div className="text-center">
        <p className="font-heading font-bold text-white text-sm sm:text-base leading-tight">{label}</p>
        <p className="font-body text-[10px] text-[#00D4FF] tracking-[0.18em] uppercase mt-0.5">{sub}</p>
      </div>
    </motion.div>
  );
}
...
function FlowArrow({ delay }) {
  return (
    <div className="flex-1 hidden sm:flex items-center justify-center relative mx-1">
      ...
      <motion.span
        className="absolute text-[#00D4FF] text-xs font-bold"
        animate={{ x: ['-8px', '8px', '-8px'], opacity: [0.4, 1, 0.4] }}
    ...
    <div className="relative w-full rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, #070E1A 0%, #0A192F 50%, #0D2145 100%)', minHeight: '340px' }}>
      ...
      >
        <div className="inline-flex items-center gap-2 mb-1">
          ...
          <span className="font-body text-[9px] tracking-[0.25em] uppercase text-[#00D4FF]/70">1Market.ph</span>
          <div className="h-px w-8 bg-[#00D4FF]/40" />
        </div>
      ...
      <div className="relative z-10 flex items-center justify-center gap-2 sm:gap-4 px-6 pb-6 pt-4">
        <NodeCircle icon="🛒" label="CUSTOMER" sub="1 GOAL" glowColor="#00D4FF" delay={0.4} />
        <FlowArrow delay={0.3} />
        <NodeCircle icon="💰" label="CASH" sub="1 VISION" glowColor="#22d3ee" delay={0.6} />
        <FlowArrow delay={0.5} />

        {/* CYCLE node with rotating ring */}
        <div className="flex flex-col items-center gap-2 z-10">
          ...
          >
            {/* Rotating outer arrow ring */}
            <motion.div
              animate={{ rotate: 360 }}
              transition={{ duration: 8, repeat: Infinity, ease: 'linear' }}
              className="absolute inset-0"
            >
              {['top-0 left-1/2 -translate-x-1/2', 'bottom-0 left-1/2 -translate-x-1/2', 'top-1/2 right-0 -translate-y-1/2'].map((pos, i) => (
                <div key={i} className={`absolute ${pos} text-[#00D4FF] text-xs`}>›</div>
              ))}
            ...
            <div
              ...
              style={{
                ...
                boxShadow: '0 0 20px 4px rgba(37,99,235,0.35)',
              }}
            >
          ...
          <div className="text-center">
            <p className="font-heading font-bold text-white text-sm sm:text-base leading-tight">CYCLE</p>
            <p className="font-body text-[10px] text-[#00D4FF] tracking-[0.12em] uppercase mt-0.5">1 MARKET VALUES</p>
          </div>
        </div>
      </div>
