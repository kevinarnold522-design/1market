...
const SUBS_BY_TYPE = {
  ...
  services: [
    ...
    'Cybersecurity', 'Cloud Services', 'Custom Development', 'Chatbot Creation', 'Data Analytics',
    ...
    // Other
    'Travel & Tours', 'Gaming Services', 'Digital Services', 'Other / Type Manually',
  ],
...
const SLIDESHOW_ANIMATIONS = [
  { value: 'fade',   label: 'Fade',   emoji: 'fade', desc: 'Crossfade' },
...
export default function AddListingModal({ onClose, defaultType = '', defaultSubcategory = '', user }) {
  ...
  const savePublishedTemplate = async () => {
    if (!effectiveUser?.id || !templateName.trim()) return;
  ...
  const prePublishRecommendations = [
    ...
    !form.image_url && totalImages === 0 && 'Add photos when possible to improve buyer trust.',
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 2 && (
                <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                  ...
                  <div className={`p-3 rounded-xl border space-y-2 ${dpaAccepted ? 'border-green-500/30 bg-green-500/5' : 'border-amber-500/30 bg-amber-500/5'}`}>
                    ...
                    <div className="max-h-28 overflow-y-auto pr-1">
                      <p className="font-body text-[10px] text-white/50 leading-relaxed">
                        ...
                        <br /><br />
                        I understand that verified seller listings can publish immediately, while other listings may be reviewed before going live, and 1MarketPH may reject or remove listings that break these rules.
                      </p>
                    </div>
                  ...
                  <p className="font-body text-[10px] text-white/25 text-center">Verified sellers publish immediately. Other listings may be reviewed before going live.</p>
